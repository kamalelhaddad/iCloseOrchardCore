...
var ImageInput = $.extend({}, Input, {
	...
	events: [
		...
		["click", "onClick", "input[type=button]"],
	],
	...
	onClick: function (event, node) {
		//btn = event.target;
		//event.data.element.trigger('propertyChange', [btn, this]);
		//obj = JSON.parse(JSON.stringify(obj));
		var msg = event.data.element.closest('.section').data().section;
		try {
			window.parent.postMessage({ message: msg }, window.location.origin);
		} catch (e) {
			//window.frames[0].postMessage({ message: msg }, window.location.origin);
			$("#mediaApp").detach().appendTo('#mediaModalBody .modal-body');
			$("#mediaApp").show();
			//mediaApp.selectedMedias = [];
			$('#mediaModalBody').modal();
		...
		function imageIsLoaded(e) {
			...
			window.parent.postMessage({ message: 'propertyChanged' }, window.location.origin);
			return;//remove this line to enable php upload
			//var antiForgeryToken = window.parent.document.getElementsByName('__RequestVerificationToken')[0].defaultValue;
			...
			//            $.ajax({
			//                type: "POST",
			//                url: '/Admin/Media/Upload',//set your server side upload script url
			//                data: formData,
			//                processData: false,
			//                contentType: false,
			...
			//});
			$(this).fileupload({
				dropZone: $(window.parent.document.getElementsById('mediaApp')),
				...
				formData: function () {
					...
					$.each(data.result.files, function (index, file) {
						if (!file.error) {
							window.parent.mediaApp.mediaItems.push(file);
						}
					});
				}
			});
